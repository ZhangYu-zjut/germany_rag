...
复杂度取决于**维度数量**，而不是问题内容：
- **简单问题** = 单一时间 + 单一对象 + 无变化/对比分析
- **复杂问题** = 多时间 或 多对象 或 变化/对比/趋势分析
...
| 模式 | 示例 | 判断 |
|-----|------|------|
| 单一年份 | "2019年", "im Jahr 2019" | ✓ 简单 |
| 无时间 | "CDU的立场" | ✓ 简单 |
| 时间跨度 | "2015-2018年", "von 2015 bis 2018" | ✗ 复杂 |
| "以来" | "seit 2015", "2015年以来" | ✗ 复杂 |
| 变化暗示 | "Veränderungen", "变化", "Wandel" | ✗ 复杂 |
...
| 模式 | 示例 | 判断 |
|-----|------|------|
| 单一党派 | "CDU/CSU", "die Grünen" | ✓ 简单 |
| 单一人物 | "Merkel", "Scholz" | ✓ 简单 |
| 整体议会 | "der Bundestag", "联邦议会" | ✓ 简单 |
| 多党派 | "verschiedene Parteien", "不同党派" | ✗ 复杂 |
| 对比 | "CDU vs SPD", "im Vergleich zu" | ✗ 复杂 |

**维度3: 分析类型**

| 类型 | 关键词 | 判断 |
|-----|-------|------|
| 陈述事实 | Position, Standpunkt, Haltung, 立场, 观点 | ✓ 简单 |
| 变化分析 | Veränderung, Wandel, 变化, 演变 | ✗ 复杂 |
| 对比分析 | Vergleich, Unterschied, 对比, 差异 | ✗ 复杂 |
| 趋势分析 | Trend, Entwicklung, 趋势, 发展 | ✗ 复杂 |

【判断规则】

```
IF 时间维度=单一 AND 对象维度=单一 AND 分析类型=陈述事实:
    → 简单
ELSE:
    → 复杂
```
...
**示例1 - 德语简单问题（必须判断为"简单"）**
问题: "Was war die Position der CDU/CSU zur Flüchtlingspolitik im Jahr 2019?"
- 时间: im Jahr 2019 → 单一年份 ✓
- 对象: CDU/CSU → 单一党派 ✓
- 类型: Position → 陈述事实 ✓
**→ 复杂度: 简单**

**示例2 - 德语简单问题（必须判断为"简单"）**
问题: "Welche Themen diskutierte der Bundestag 2020?"
- 时间: 2020 → 单一年份 ✓
- 对象: der Bundestag → 整体议会 ✓
- 类型: Themen → 陈述事实 ✓
**→ 复杂度: 简单**
...
- 时间: seit 2015 → 时间跨度 ✗
- 类型: Veränderungen → 变化分析 ✗
**→ 复杂度: 复杂**

**示例5 - 中文复杂问题（必须判断为"复杂"）**
问题: "在2015-2018年期间,不同党派在难民问题上的立场有何变化?"
- 时间: 2015-2018年 → 时间跨度 ✗
- 对象: 不同党派 → 多党派 ✗
- 类型: 变化 → 变化分析 ✗
**→ 复杂度: 复杂**
